...
class GetTaskDateUseCase @Inject constructor(private val repository: FireBaseRepository) {

    suspend operator fun invoke(currentUser: String, constructionName: String): Flow<List<String>> {
        return flow {
            val result = repository.getTaskDate(currentUser, constructionName)
