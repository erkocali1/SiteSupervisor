...
class GetAllPostUseCase @Inject constructor(private val repository: FireBaseRepository) {

    suspend operator fun invoke(currentUser: String, constructionName: String): Flow<List<DataModel>> {
        return flow {
            val result = repository.getAllPost(currentUser, constructionName)
