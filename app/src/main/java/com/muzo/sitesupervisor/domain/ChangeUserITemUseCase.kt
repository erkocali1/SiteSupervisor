...
class ChangeUserITemUseCase @Inject constructor(private val repository: FireBaseRepository) {

    suspend operator fun invoke(itemValue: String, currentUser: String, changedItem: String): Flow<Unit>{
        return flow {
            val result=repository.changeUserItem(itemValue, currentUser, changedItem)
