...
class GetSiteSuperVisorInfoUseCase @Inject constructor(private val repository: FireBaseRepository) {

    suspend operator fun invoke(siteSupervisor:String):Flow<UserInfo>{

        return flow {
            val result=repository.getSiteSuperVisorInfo(siteSupervisor)
